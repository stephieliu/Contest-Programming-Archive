...
public class GamePanel extends JPanel implements Runnable, KeyListener{
  ...
  public GamePanel(){
    ...
    //display the secondObject
    obstacle = new secondObject((int) (Math.floor(Math.random()*(GAME_WIDTH))));
  ...
  public void checkCollision(){
    ...
    if(obstacle.y >= GAME_HEIGHT - secondObject.BALL_DIAMETER){
        Score.score++;
        obstacle.y = 0;
        obstacle.x = (int) (Math.floor(Math.random()*(GAME_WIDTH-secondObject.BALL_DIAMETER)));
        obstacle.yVelocity+=1;
    }
    //removes a point whenever you hit the ball
    if(obstacle.intersects(ball)){
        obstacle.y = 0;
        obstacle.x = (int) (Math.floor(Math.random() * (GAME_WIDTH - secondObject.BALL_DIAMETER)));
        Score.score--;
    }
